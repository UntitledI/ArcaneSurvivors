		...
		if (patrol.showGizmo) {
			...
			for (int i = 0; i < patrol.patrolList.Count; i++) {
				...
				for (int j = 0; j < patrol.patrolList [i].wayPoints.Count; j++) {
					if (patrol.patrolList [i].wayPoints [j] != null) {
						...
						if (patrol.useHandleForVertex) {
							...
							var fmh_78_61_638146060112584752 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, patrol.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
				...
				if (patrol.useHandleForVertex) {
					...
					var fmh_99_59_638146060112608012 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, patrol.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
