			...
			for (int i = 0; i < manager.waypointList.Count; i++) {
				if (manager.waypointList [i] != null) {
					...
					if (manager.useHandleForVertex) {
						...
						var fmh_56_60_638146060112585049 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
