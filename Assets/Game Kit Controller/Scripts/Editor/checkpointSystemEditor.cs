			...
			for (int i = 0; i < manager.checkPointList.Count; i++) {
				if (manager.checkPointList [i] != null) {
					...
					if (manager.useHandleForVertex) {
						...
						var fmh_67_60_638146060112588077 = Quaternion.identity; Vector3 newPoint = Handles.FreeMoveHandle (oldPoint, manager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
