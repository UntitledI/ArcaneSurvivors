						...
						for (int i = 0; i < builder.eventTriggerList.Count; i++) {
							if (builder.eventTriggerList [i]) {
								...
								if (builder.useHandleForVertex) {
									...
									var fmh_117_55_638146060112800789 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, builder.handleRadius, snapValue, Handles.CircleHandleCap);
					...
					for (int i = 0; i < builder.verticesPosition.Count; i++) {
						if (builder.verticesPosition [i]) {
							...
							if (builder.useHandleForVertex) {
								...
								var fmh_168_54_638146060112802504 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, builder.handleRadius, snapValue, Handles.CircleHandleCap);
								if (EditorGUI.EndChangeCheck ()) {
