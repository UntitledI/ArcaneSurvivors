				...
				for (int i = 0; i < pathManager.wayPoints.Count; i++) {
					...
					if (currentWaypoint) {
						...
						if (pathManager.useHandleForVertex) {
							...
							var fmh_172_53_638146060113204470 = Quaternion.identity; newPoint = Handles.FreeMoveHandle (oldPoint, pathManager.handleRadius, new Vector3 (.25f, .25f, .25f), Handles.CircleHandleCap);
