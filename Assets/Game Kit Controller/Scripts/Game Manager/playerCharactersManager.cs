...
public class playerCharactersManager : MonoBehaviour
{
	...
	public void updateCanvasValuesByPlayer (GameObject pauseManagerObject, GameObject newCanvasPanel)
	{
		bool activatingMultiplayer = currentNumberOfPlayers > 1;
			...
			for (int i = 0; i < playerList.Count; i++) {
				...
				if (currentPauseManager.gameObject == pauseManagerObject) {
					...
					if (currentCanvas != null) {
						...
						menuPause.canvasInfo currentCanvasInfo = currentPauseManager.canvasInfoList [currentPauseManager.canvasInfoList.Count - 1];
