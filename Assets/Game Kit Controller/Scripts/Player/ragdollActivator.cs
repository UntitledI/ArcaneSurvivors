	...
	void LateUpdate ()
	{
		if (currentState == ragdollState.blendToAnim) {
			if (Time.time <= ragdollingEndTime + mecanimToGetUpTransitionTime) {
				...
				Debug.DrawLine (newRootPosition + mainTransform.up, newRootPosition + mainTransform.up + Vector3.up * 100, Color.green, 2);
				...
				Debug.DrawLine (newRootPosition, newRootPosition + Vector3.up * 100, Color.blue, 2);
